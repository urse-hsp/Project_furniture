<template>
  <div class="wrap">
    <modalBox :title="title" :boole="boole"></modalBox>
    <header>
      <span @click="login"><i class="el-icon-arrow-left"></i></span>
    ...
    <div class="login">
      ...
      <div class="account">
        <form action="">
          ...
          <p><input type="password" placeholder="请设置6-20位密码,包含字母、数字或符号" id="password"/></p>
          <p><input type="text" placeholder="请输入图形验证码"/></p>
          <p><input type="text" placeholder="请输入手机验证码"/><a href="#" class="forget">获取验证码</a></p>
        </form>
      </div>
      <div class="agreement">
        <input type="checkbox" id="agreement"/>阅读并同意 <a href="#">《丽芙家居用户协议》</a>和 <a href="#">《隐私声明》</a>
      </div>
      <div class="btn">
        <button @click="register">注册</button> <router-link to="/login"><button>登录</button></router-link>
      </div>
    </div>
  </div>
</template>
